	...
	for (my $j=0;$j<scalar(@go);$j++) #for go value
	{
		...
		#{
			$data=$unaligned; #Let data = unaligned data
				$g=$go[$j]; #Let g=go[j]
#				$e=$ge[$k];
			system("perl nw_BLOS.pl $data $g >| nwblo_alignment"); #call NW(g).pl on
				$err = `perl alignment_accuracy.pl $aligned nwblo_alignment` ;#Get error
				$error[$j] += $err;
		#}
	}
...
print "The minimum error is: $minError, $maxG\n";
